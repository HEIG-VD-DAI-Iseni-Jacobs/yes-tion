...
public class NotesController {
    ...
    /**
     * POST /notes
     * Body attendu : { "noteTitle": "...", "noteContent": "..." }
     * Nécessite le cookie "user".
     * Réponses :
     *   201 si création
     *   400 si corps invalide
     *   401 si utilisateur non connecté
     *   409 si la noteTitle existe déjà (on considère "unique" dans tout le système ou pour le même user)
     */
     ...
     *   400 si corps invalide
     *   401 si utilisateur non connecté
     *   404 si la note n'existe pas OU n'appartient pas à l'utilisateur
     *   409 si le nouveau titre existe déjà
     */
     ...
     * GET /notes
     * Nécessite le cookie "user".
     * Renvoie la liste de toutes les notes de l'utilisateur.
     * Réponses :
     *   200 si ok
     *   401 si pas connecté
     */
    public void getAllNotes(Context ctx) {
        ...
        ctx.json(notes.values().stream()
                .filter(note -> note.userId.equals(currentUserId))
                .toArray());
    }
     ...
     * GET /notes/{noteId}
     * Nécessite le cookie "user".
     * Renvoie la note si elle appartient à l'utilisateur.
     * Réponses :
     *   200 si ok
     *   401 si pas connecté
     *   404 si la note n'existe pas ou n'appartient pas à l'utilisateur
